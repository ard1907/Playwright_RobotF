...
- `tests/ui/ts_05_register_cards_generic.robot`
  ...
  - Laufmodi:
    - Normalmodus: `robot tests/ui/ts_05_register_cards_generic.robot`
      - Lädt ein Fixture, überspringt bei `first_run.completed = false` und prüft den Live-Dialog gegen die erwarteten Daten.
    - First-Run-Modus: `robot --include first-run --variable ENABLE_FIRST_RUN_TESTS:True tests/ui/ts_05_register_cards_generic.robot`
