...
- `ts_05_register_cards_generic.robot` verifies generic register cards through YAML fixtures such as `bva.yaml` and `dguv.yaml`.
- `ts_05b_register_cards_generic.robot` verifies generic register cards through decrypted API-response fixtures such as `bva.json` and `dguv.json`.
